...
class CompanyPatch implements ModelInterface, ArrayAccess
{
    ...
    public function setName($name)
    {
        if (!is_null($name) && (strlen($name) > 64)) {
            throw new \InvalidArgumentException('invalid length for $name when calling CompanyPatch., must be smaller than or equal to 64.');
        }
    ...
    public function setBillName($bill_name)
    {
        if (!is_null($bill_name) && (strlen($bill_name) > 64)) {
            throw new \InvalidArgumentException('invalid length for $bill_name when calling CompanyPatch., must be smaller than or equal to 64.');
        }
